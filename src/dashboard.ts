...
const PentwheelDashboard: React.FC = () => {
  ...
  const MetricCard: React.FC<MetricCardProps> = ({ icon, title, value, subtitle, gradientClass }) => (
    <div className="bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300 border border-white border-opacity-20">
  ...
  const StatusBadge: React.FC<StatusBadgeProps> = ({ status, statusType }) => (
    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusBadgeClass(statusType)}`}>
      {status}
    </span>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-purple-600">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white bg-opacity-95 backdrop-blur-sm rounded-2xl p-6 shadow-lg">
            <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              🌏 Incoming Overseas Shipments
            </h2>
            <div className="overflow-x-auto">
